...
export class UpdateTransactionRules {
  ...
  async process(
    ...
    newTransactionDto: UpdateTransactionDto,
    userId: string,
  ) {
  ...
  private async revertBalance(transaction: Transaction, userId: string) {
    if (transaction.type === 'INCOME') {
      ...
      const source = await this.ownership.validateAccountOwnership(
        transaction.sourceAccountId!,
        userId,
      );
      const destination = await this.ownership.validateAccountOwnership(
        transaction.destinationAccountId!,
        userId,
      );
      await this.balance.revertTransfer(
        source,
        destination,
  ...
  private async applyNewBalance(transaction: Transaction, userId: string) {
    if (transaction.type === 'INCOME') {
      ...
      await this.balance.applyTransfer(
        source,
        destination,
        Number(transaction.amount),
      );
